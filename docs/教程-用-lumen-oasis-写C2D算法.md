...
内置 7 个模板,都是纯 Python stdlib、**只输出聚合**(原始行永不离开数据边界):
...
目前 demo 市场上由 `lumen oasis` 模板上市的算法:`colstats`、`histogram`、`quantiles`、`correlation`(corrtest)、`linreg`、`logreg`、`dpstats` —— **七个都 approved + trusted**。
