...
- Code Artifact bytes are downloaded by authenticated Run ID plus Artifact ID, never a user path. The response uses a sanitized attachment filename and `nosniff`; cross-owner and unknown IDs both return not found.
