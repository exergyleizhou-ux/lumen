...
func (r *Registry) Get(name, version string) (*Artifact, bool) {
	r.mu.Lock()
	defer r.mu.Unlock()
	a, ok := r.artifacts[name+":"+version]
	return a, ok
}
func (r *Registry) List() []*Artifact { r.mu.Lock(); defer r.mu.Unlock(); return r.listLocked() }

// listLocked assumes the caller holds the lock.
func (r *Registry) listLocked() []*Artifact {
	var out []*Artifact
...
func (r *Registry) FindByTag(tag string) []*Artifact {
	r.mu.Lock()
	defer r.mu.Unlock()
	var out []*Artifact
	for _, a := range r.artifacts {
...
func (r *Registry) FormatArtifacts() string {
	r.mu.Lock()
	defer r.mu.Unlock()
