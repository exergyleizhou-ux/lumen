...
func NewGraph() *Graph {
	return &Graph{nodes: map[string]*Node{}, edges: map[string]*Edge{}, fromIndex: map[string][]string{}, toIndex: map[string][]string{}}
}
...
func (g *Graph) AddEdge(id, from, to, label string, props map[string]any) *Edge {
	g.mu.Lock()
	defer g.mu.Unlock()
	e := &Edge{ID: id, From: from, To: to, Label: label, Properties: props}
	g.edges[id] = e
	g.fromIndex[from] = append(g.fromIndex[from], id)
	g.toIndex[to] = append(g.toIndex[to], id)
	return e
}
...
func (g *Graph) Outgoing(nodeID string) []*Edge {
	...
	defer g.mu.RUnlock()
	var out []*Edge
	for _, eid := range g.fromIndex[nodeID] {
...
func (g *Graph) Incoming(nodeID string) []*Edge {
	...
	defer g.mu.RUnlock()
	var out []*Edge
	for _, eid := range g.toIndex[nodeID] {
		if e, ok := g.edges[eid]; ok {
			out = append(out, e)
		}
	}
...
func (g *Graph) Neighbors(nodeID string) []*Node {
	...
	for _, eid := range g.fromIndex[nodeID] {
		if e, ok := g.edges[eid]; ok {
			if n, ok2 := g.nodes[e.To]; ok2 && !seen[n.ID] {
				seen[n.ID] = true
				out = append(out, n)
			}
		}
	...
	for _, eid := range g.toIndex[nodeID] {
		if e, ok := g.edges[eid]; ok {
			if n, ok2 := g.nodes[e.From]; ok2 && !seen[n.ID] {
				seen[n.ID] = true
				out = append(out, n)
			}
		}
...
func (g *Graph) ShortestPath(from, to string) []string {
	...
	for len(queue) > 0 {
		...
		for _, eid := range g.fromIndex[current] {
			if e, ok := g.edges[eid]; ok {
				if !visited[e.To] {
					visited[e.To] = true
					prev[e.To] = current
...
func (g *Graph) HasCycle() bool {
	...
	visit = func(n string) bool {
		delete(white, n)
		gray[n] = true
		for _, eid := range g.fromIndex[n] {
			if e, ok := g.edges[eid]; ok {
				if gray[e.To] {
					return true
				}
				if white[e.To] && visit(e.To) {
					return true
				}
			}
		}
...
func (g *Graph) SubGraph(nodeIDs []string) *Graph {
	...
	for _, e := range g.edges {
		if _, ok1 := sg.nodes[e.From]; ok1 {
			if _, ok2 := sg.nodes[e.To]; ok2 {
				sg.edges[e.ID] = e
				sg.fromIndex[e.From] = append(sg.fromIndex[e.From], e.ID)
				sg.toIndex[e.To] = append(sg.toIndex[e.To], e.ID)
			}
		}
	}
	return sg
}
func (g *Graph) FormatStats() string {
	g.mu.RLock()
	defer g.mu.RUnlock()
	...
	var degrees []nc
	for id := range g.nodes {
		deg := len(g.fromIndex[id]) + len(g.toIndex[id])
		degrees = append(degrees, nc{id, deg})
	}
