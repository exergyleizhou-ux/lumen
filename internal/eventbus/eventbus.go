...
func (b *Bus) Publish(e Event) {
	...
	if len(b.history) > b.maxH {
		b.history = b.history[len(b.history)-b.maxH:]
	}
...
func (b *Bus) History(t string, limit int) []Event {
	b.mu.RLock()
	defer b.mu.RUnlock()
	var o []Event
	for i := len(b.history) - 1; i >= 0 && len(o) < limit; i-- {
		if t == "" || b.history[i].Type == t {
			o = append(o, b.history[i])
		}
	}
	return o
}
...
func (e *Emitter) Emit(et string, data map[string]any) {
	e.bus.Publish(Event{ID: fmt.Sprintf("evt-%d", time.Now().UnixNano()), Type: et, Source: e.source, Data: data})
}
