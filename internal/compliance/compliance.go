...
import ("fmt";"regexp";"sort";"strings";"sync")

type Rule struct{ID,Name,Category,Description string;Check func(data map[string]any)Result}
type Result struct{RuleID string;Pass bool;Severity string;Message string;Evidence string}
type Report struct{mu sync.Mutex;results []Result;passed,failed int}
func NewReport()*Report{return &Report{}}
func(r*Report)Record(res Result){r.mu.Lock();defer r.mu.Unlock();r.results=append(r.results,res);if res.Pass{r.passed++}else{r.failed++}}
func(r*Report)AllPass()bool{r.mu.Lock();defer r.mu.Unlock();return r.failed==0}
func(r*Report)Summary()string{r.mu.Lock();defer r.mu.Unlock();return fmt.Sprintf("Compliance: %d passed, %d failed (total %d)",r.passed,r.failed,len(r.results))}
...
func(s*Scanner)Scan(data map[string]any)*Report{
  rpt:=NewReport()
  for _,rule:=range s.rules{res:=rule.Check(data);res.RuleID=rule.ID;rpt.Record(res)}
  return rpt
}

func DefaultRules()[]Rule{return []Rule{
  {ID:"GDPR-001",Name:"PII Detection",Category:"GDPR",Description:"No PII in agent output",
    Check:func(d map[string]any)Result{
      for _,v:=range d{if s,ok:=v.(string);ok{if detectPII(s){return Result{Pass:false,Severity:"high",Message:"PII detected in output"}}}}
  ...
  {ID:"SEC-001",Name:"No Secrets",Category:"Security",Description:"No API keys in output",
    Check:func(d map[string]any)Result{
      for _,v:=range d{if s,ok:=v.(string);ok{if hasSecret(s){return Result{Pass:false,Severity:"critical",Message:"API key or token detected"}}}}
      return Result{Pass:true}
    }},
}}
...
func FormatReport(rpt *Report)string{
  rpt.mu.Lock();defer rpt.mu.Unlock()
  ...
  for _,r:=range rpt.results{icon:="✅";if !r.Pass{icon="❌"};fmt.Fprintf(&sb,"%s %s [%s]: %s\n",icon,r.RuleID,r.Severity,r.Message)}
