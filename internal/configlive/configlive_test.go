...
func TestStoreWatch(t *testing.T){s:=NewStore();called:=false;s.Watch("k",func(k string,o,n any){called=true});s.Set("k","v","");if!called{t.Error("watch")}}
func TestRollback(t *testing.T){s:=NewStore();s.Set("k","first","");s.Set("k","second","");s.Rollback("k");v,_:=s.Get("k");if v!="first"{t.Error("rollback")}}
