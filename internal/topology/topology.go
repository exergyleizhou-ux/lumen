...
func (g *Graph) Nodes() []*Node {
	...
	for _, n := range g.nodes { out = append(out, n) }
...
func (g *Graph) Outgoing(nodeID string) []*Edge {
	g.mu.RLock(); defer g.mu.RUnlock()
	var out []*Edge
	for _, e := range g.edges { if e.From == nodeID { out = append(out, e) } }
	return out
}
...
func (g *Graph) Incoming(nodeID string) []*Edge {
	g.mu.RLock(); defer g.mu.RUnlock()
	var out []*Edge
	for _, e := range g.edges { if e.To == nodeID { out = append(out, e) } }
	return out
}
...
func (g *Graph) CriticalPath() ([]string, float64) {
	...
	for id := range g.nodes { inDeg[id] = 0 }
	...
	for id := range g.nodes { dist[id] = -1 }
	for _, id := range order { dist[id] = 0 }
	...
	maxDist := -1.0
	for id, d := range dist {
		if d > maxDist { maxDist = d; end = id }
	}
	if maxDist < 0 { return nil, 0 }

	path := []string{end}
	for prev[end] != "" { end = prev[end]; path = append([]string{end}, path...) }
	return path, maxDist
}
...
func (g *Graph) SPOF() []string {
	...
	for _, e := range g.edges { result[e.To] = true }
	for _, e := range g.edges {
		if len(g.Incoming(e.To)) > 1 { delete(result, e.To) }
	}
	var out []string
	for id := range result { out = append(out, id) }
	sort.Strings(out)
	return out
}
...
func (g *Graph) HubNodes(n int) []string {
	g.mu.RLock(); defer g.mu.RUnlock()
	deg := map[string]int{}
	for _, e := range g.edges { deg[e.From]++; deg[e.To]++ }
	type kv struct{ id string; deg int }
	var pairs []kv
	for id, d := range deg { pairs = append(pairs, kv{id, d}) }
	sort.Slice(pairs, func(i, j int) bool { return pairs[i].deg > pairs[j].deg })
	var out []string
	for i := 0; i < n && i < len(pairs); i++ { out = append(out, pairs[i].id) }
	return out
}
...
func (g *Graph) ToJSON() ([]byte, error) {
	...
	o := out{Edges: g.edges}
	for _, n := range g.nodes { o.Nodes = append(o.Nodes, n) }
...
func (g *Graph) FormatGraph() string {
	...
	if g.HasCycle() { sb.WriteString("\n⚠️  Cycle detected!\n") }
	spof := g.SPOF()
	if len(spof) > 0 { fmt.Fprintf(&sb, "\n🔴 SPOFs: %v\n", spof) }
	cp, w := g.CriticalPath()
	if len(cp) > 0 { fmt.Fprintf(&sb, "\nCritical Path (%.1f): %s\n", w, strings.Join(cp, " → ")) }
	return sb.String()
}
