...
func (s *Store) Set(key string, enabled bool, rollout int) *Flag {
	s.mu.Lock()
	defer s.mu.Unlock()
	f := &Flag{Key: key, Enabled: enabled, Rollout: rollout, UpdatedAt: time.Now()}
	s.flags[key] = f
	return f
}
func (s *Store) AddRule(key, field, op, value string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if f, ok := s.flags[key]; ok {
		f.Rules = append(f.Rules, Rule{Field: field, Op: op, Value: value})
	}
}
func (s *Store) IsEnabled(key string, ctx map[string]string) bool {
	s.mu.RLock()
	f, ok := s.flags[key]
	s.mu.RUnlock()
	if !ok || !f.Enabled {
...
func (s *Store) FormatFlags() string {
	flags := s.List()
	if len(flags) == 0 {
		return "No feature flags.\n"
	}
	var sb strings.Builder
	fmt.Fprintf(&sb, "Feature Flags (%d):\n\n", len(flags))
	for _, f := range flags {
		icon := "○"
		if f.Enabled {
			icon = "●"
		}
		fmt.Fprintf(&sb, "%s %-25s rollout:%-3d%% rules:%d\n", icon, f.Key, f.Rollout, len(f.Rules))
	}
	return sb.String()
}
