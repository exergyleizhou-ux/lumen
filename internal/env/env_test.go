...
func TestDumpSecrets(t *testing.T) {
	...
	dump := s2.Dump()
	if val, ok := dump["TOKEN"]; ok && strings.Contains(val, "****") { t.Log("masked") }
	os.Unsetenv("TOKEN")
}
func TestReporter(t *testing.T) {
	r := NewReporter()
	info := r.Info()
	if info["hostname"] == "" { t.Error("hostname") }
}
