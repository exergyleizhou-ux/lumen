...
func NewNotary() *Notary {
	return &Notary{keys: map[string]*KeyPair{}, attestations: map[string][]*Attestation{}, maxAttest: 1000}
}
...
func (n *Notary) Sign(keyID, subject string, claims map[string]any, ttl time.Duration) (*Attestation, error) {
	...
	if len(n.attestations[subject]) > n.maxAttest {
		n.attestations[subject] = n.attestations[subject][1:]
	}
	n.mu.Unlock()
...
func (n *Notary) Verify(att *Attestation) (bool, string) {
	...
	if !att.ExpiresAt.IsZero() && time.Now().After(att.ExpiresAt) {
	...
	if !kp.Verify(payload, att.Signature.Signature) {
...
func (hc *HashChain) VerifyChain() (bool, []string) {
	...
	for _, e := range hc.entries {
		if e.PrevHash != prev {
			issues = append(issues, fmt.Sprintf("chain break at index %d", e.Index))
		}
		h := sha256.New()
		raw := fmt.Sprintf("%d:%s:%s:%d", e.Index, prev, e.Data, e.Timestamp.UnixNano())
		h.Write([]byte(raw))
		if hex.EncodeToString(h.Sum(nil)) != e.Hash {
			issues = append(issues, fmt.Sprintf("hash mismatch at index %d", e.Index))
		}
		prev = e.Hash
	}
...
func truncHex(h string, n int) string {
	if len(h) <= n {
		return h
	}
	return h[:n] + "..."
