...
const (
	...
	NodeFunc   NodeType = "FuncDecl"
	NodeStruct NodeType = "StructDecl"
	NodeCall   NodeType = "CallExpr"
	NodeIdent  NodeType = "Ident"
...
func NewNode(typ NodeType, name string) *Node {
	return &Node{Type: typ, Name: name, Attrs: map[string]string{}}
}
func (n *Node) AddChild(child *Node) { n.Children = append(n.Children, child) }
func (n *Node) SetAttr(k, v string)  { n.Attrs[k] = v }
func (n *Node) FindAllType(typ NodeType) []*Node {
	var out []*Node
	if n.Type == typ {
		out = append(out, n)
	}
	for _, c := range n.Children {
		out = append(out, c.FindAllType(typ)...)
	}
	return out
}
func (n *Node) Walk(fn func(*Node, int) bool) {
	if !fn(n, 0) {
		return
	}
	for _, c := range n.Children {
		c.Walk(func(node *Node, depth int) bool { return fn(node, depth+1) })
	}
}
...
func (t *Tree) FindByType(typ NodeType) []*Node {
	t.mu.Lock()
	defer t.mu.Unlock()
	return t.root.FindAllType(typ)
}
func (t *Tree) reindex() {
	...
	t.root.Walk(func(n *Node, depth int) bool { t.index[n.Name] = append(t.index[n.Name], n); return true })
}
func (t *Tree) FormatTree() string {
	...
	sb.WriteString("\n\n")
	symbols := make([]string, 0, len(t.index))
	for k := range t.index {
...
func (t *Tree) CountNodes() int {
	...
	defer t.mu.Unlock()
	count := 0
	t.root.Walk(func(n *Node, depth int) bool { count++; return true })
	return count
}
