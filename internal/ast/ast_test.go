...
func TestTree(t *testing.T) {
	...
	tr := NewTree()
	tr.SetRoot(root)
	if tr.CountNodes() != 3 { t.Error("node count") }
	funcs := tr.FindByType(NodeFunc)
	if len(funcs) != 1 { t.Error("func count") }
	nodes := tr.FindByName("main")
	if len(nodes) != 1 { t.Error("find by name") }
}
func TestWalk(t *testing.T) {
	...
	if count != 3 { t.Error("walk count") }
}
func TestFormat(t *testing.T) {
	tr := NewTree()
	tr.SetRoot(NewNode(NodeFile, "test.go"))
	if tr.FormatTree() == "" { t.Error("format") }
}
