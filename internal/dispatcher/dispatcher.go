...
func NewDispatcher() *Dispatcher {
	return &Dispatcher{workers: map[string]Worker{}, busyWorkers: map[string]bool{}, maxResults: 1000, stopCh: make(chan struct{})}
}
...
func (d *Dispatcher) Results() []TaskResult {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func (d *Dispatcher) FormatStatus() string {
	...
	busy := 0
	for _, b := range d.busyWorkers {
		if b {
			busy++
		}
	}
	fmt.Fprintf(&sb, "  Busy workers: %d\n", busy)
	if d.queue.Len() > 0 {
		fmt.Fprintf(&sb, "\n  Pending tasks:\n")
		for i := 0; i < min(d.queue.Len(), 5); i++ {
			t := d.queue[i]
			fmt.Fprintf(&sb, "    [%d] %s (pri=%d)\n", i, t.Name, t.Priority)
		}
	}
	return sb.String()
}
