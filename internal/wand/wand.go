...
func (w *Wand) Diagnose() ([]Issue, error) {
	...
	for _, d := range diags {
		...
		if err != nil {
			return all, fmt.Errorf("%s: %w", d.Name, err)
		}
		for i := range issues {
			issues[i].ID = fmt.Sprintf("%s-%03d", d.Name, i+1)
		}
...
func (w *Wand) Fix(issues []Issue) []FixResult {
	...
	for _, is := range issues {
		...
		if auto && sevOrder[is.Severity] >= sevOrder[maxSev] && !is.AutoFix {
			continue
		}
...
func FormatIssues(issues []Issue) string {
	...
	for _, is := range issues {
		...
		fmt.Fprintf(&sb, "   💡 %s\n", is.Suggestion)
		if is.AutoFix {
			sb.WriteString("   🔧 Auto-fix available\n")
		}
		if is.Description != "" {
			fmt.Fprintf(&sb, "   📝 %s\n", is.Description)
		}
