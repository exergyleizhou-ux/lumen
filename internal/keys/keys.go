...
func NewManager() *Manager {
	return &Manager{keys: map[string]*Key{}, rotationInterval: 90 * 24 * time.Hour}
}
...
func (m *Manager) Rotate(id string) (*Key, ed25519.PrivateKey, error) {
	...
	m.mu.Unlock()
	if !ok {
		return nil, nil, fmt.Errorf("key %q not found", id)
	}
...
func (m *Manager) List() []*Key { m.mu.Lock(); defer m.mu.Unlock(); return m.listLocked() }

// listLocked assumes the caller holds the lock.
func (m *Manager) listLocked() []*Key {
	var out []*Key
	for _, k := range m.keys {
		out = append(out, k)
	}
