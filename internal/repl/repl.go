...
func NewSession() *Session {
	return &Session{input: bufio.NewReader(os.Stdin), output: os.Stdout, handlers: map[string]func([]string) string{}}
}
func (s *Session) Register(name string, fn func([]string) string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.handlers[name] = fn
}
func (s *Session) Handle(cmd string) string {
	...
	if ok {
		...
		result = fmt.Sprintf("unknown command: %s (try: help)", name)
	}
	s.mu.Lock()
	s.history = append(s.history, Command{Line: cmd, Timestamp: time.Now(), Result: result})
	s.mu.Unlock()
	return result
}
func (s *Session) History() []Command {
	s.mu.Lock()
	defer s.mu.Unlock()
