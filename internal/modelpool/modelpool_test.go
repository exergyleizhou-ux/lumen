...
func TestPoolRegister(t *testing.T) {
	...
	if m.Name != "gpt4" {
		t.Errorf("expected gpt4, got %s", m.Name)
	}
}
func TestRoundRobin(t *testing.T) {
	...
	if m1.Name == m2.Name {
		t.Error("round robin should cycle")
	}
}
...
func TestCapabilityFilter(t *testing.T) {
	...
	if m.Name != "viewer" {
		t.Error("should pick vision-capable")
	}
}
func TestFailureRecording(t *testing.T) {
	p := NewPool(&RoundRobinStrategy{})
	p.Register(&Info{Name: "flaky", Model: "flaky-model", Healthy: true})
	p.RecordFailure("flaky", "timeout")
	p.RecordFailure("flaky", "timeout")
	p.RecordFailure("flaky", "timeout")
	m, err := p.Select(context.Background(), nil, "")
	if err == nil {
		t.Errorf("should not select unhealthy model, got %s", m.Name)
	}
}
