	...
	setRec = func(cur any, depth int) (any, error) {
		if depth == len(keys)-1 {
			...
			case map[string]any:
				...
				return m, nil
			default:
				return nil, fmt.Errorf("cannot set at depth %d: %T", depth, cur)
			}
		}
		switch m := cur.(type) {
		case map[string]any:
			...
			return m, nil
		default:
			return nil, fmt.Errorf("cannot traverse at %s", keys[depth])
		}
	}
...
func (e *EncodeTool) Base64Encode(input string) string {
