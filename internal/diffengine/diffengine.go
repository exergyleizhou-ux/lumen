...
type DiffOp string
const (OpAdd DiffOp = "add"; OpRemove DiffOp = "remove"; OpChange DiffOp = "change"; OpSame DiffOp = "same")

// DiffLine is one line of a diff.
type DiffLine struct {Op DiffOp `json:"op"`; OldLine string `json:"old_line,omitempty"`; NewLine string `json:"new_line,omitempty"`; OldLineNum int `json:"old_line_num"`; NewLineNum int `json:"new_line_num"`}

// DiffResult is a complete diff.
type DiffResult struct {OldFile string `json:"old_file"`; NewFile string `json:"new_file"`; Lines []DiffLine `json:"lines"`; Added int `json:"added"`; Removed int `json:"removed"`; Changed int `json:"changed"`}
...
func (e *Engine) LineDiff(oldText, newText string) *DiffResult {
	...
	for i := 1; i <= la; i++ {
		for j := 1; j <= lb; j++ {
			if oldLines[i-1] == newLines[j-1] { dp[i][j] = dp[i-1][j-1] + 1 } else {
	...
	for k := len(raw) - 1; k >= 0; k-- { result.Lines = append(result.Lines, raw[k]) }
	for _, l := range result.Lines {
		switch l.Op {
		case OpAdd: result.Added++
		case OpRemove: result.Removed++
		case OpChange: result.Changed++
		}
	}
	return result
}
...
func (e *Engine) WordDiff(lineA, lineB string) []DiffLine {
	...
	for i := 1; i <= la; i++ {
		for j := 1; j <= lb; j++ {
			if wordsA[i-1] == wordsB[j-1] { dp[i][j] = dp[i-1][j-1] + 1 } else {
				if dp[i-1][j] > dp[i][j-1] { dp[i][j] = dp[i-1][j] } else { dp[i][j] = dp[i][j-1] }
			}
		}
	}
	...
	var out []DiffLine
	for k := len(raw) - 1; k >= 0; k-- { out = append(out, raw[k]) }
	return out
}
...
type JSONChange struct {Path JSONPath `json:"path"`; Op DiffOp `json:"op"`; OldValue any `json:"old_value,omitempty"`; NewValue any `json:"new_value,omitempty"`}

// JSONDiffResult is the result of comparing two JSON documents.
type JSONDiffResult struct {Changes []JSONChange `json:"changes"`; Additions int `json:"additions"`; Removals int `json:"removals"`; Modifications int `json:"modifications"`}
...
func (e *Engine) jsonDiffRec(old, new any, path JSONPath, result *JSONDiffResult) {
	...
	if oldIsMap && newIsMap {
		for k, nv := range newMap {
			...
			if path == "" { subPath = JSONPath(k) }
			if ov, ok := oldMap[k]; ok { e.jsonDiffRec(ov, nv, subPath, result) } else {
	...
	if oldIsSlice && newIsSlice {
		maxLen := len(oldSlice)
		if len(newSlice) > maxLen { maxLen = len(newSlice) }
		for i := 0; i < maxLen; i++ {
			sp := JSONPath(fmt.Sprintf("%s[%d]", path, i))
			if i < len(oldSlice) && i < len(newSlice) { e.jsonDiffRec(oldSlice[i], newSlice[i], sp, result) } else if i < len(newSlice) {
...
func FormatDiff(result *DiffResult) string {
	...
	for _, l := range result.Lines {
		switch l.Op {
		case OpAdd: fmt.Fprintf(&sb, "+ %s\n", l.NewLine)
		case OpRemove: fmt.Fprintf(&sb, "- %s\n", l.OldLine)
		case OpChange: fmt.Fprintf(&sb, "~ %s → %s\n", l.OldLine, l.NewLine)
		case OpSame: fmt.Fprintf(&sb, "  %s\n", l.OldLine)
		}
	}
	return sb.String()
}
...
func FormatChanges(result *JSONDiffResult) string {
	...
	for _, c := range result.Changes {
		switch c.Op {
		case OpAdd: fmt.Fprintf(&sb, "  + %s = %v\n", c.Path, c.NewValue)
		case OpRemove: fmt.Fprintf(&sb, "  - %s = %v\n", c.Path, c.OldValue)
		case OpChange: fmt.Fprintf(&sb, "  ~ %s: %v → %v\n", c.Path, c.OldValue, c.NewValue)
		}
	}
	return sb.String()
}
