...
const marker = "window.CodeUI = { buildWorkbenchSnapshotV2, workbenchTargetOrigin, handleWorkbenchRefreshRequest };";
const end = source.indexOf(marker);
if (end < 0) throw new Error("CodeUI export missing");
let listener; let fetches = 0;
const parent = {};
const sandbox = { window: { parent, location: { origin: "https://lumen.test" }, addEventListener: (name, fn) => { if (name === "message") listener = fn; } }, location: { pathname: "/" }, document: { getElementById: () => null }, fetch: async () => { fetches++; return { ok: false }; }, URL };
...
listener({ source: {}, origin: "https://lumen.test", data: { kind: "lumen.workbench.refresh", version: 1, run_id: "run_valid" } });
listener({ source: parent, origin: "https://evil.test", data: { kind: "lumen.workbench.refresh", version: 1, run_id: "run_valid" } });
listener({ source: parent, origin: "https://lumen.test", data: { kind: "lumen.workbench.refresh", version: 1, run_id: "../../bad" } });
if (fetches !== 0) throw new Error("invalid refresh request reached runtime fetch");
listener({ source: parent, origin: "https://lumen.test", data: { kind: "lumen.workbench.refresh", version: 1, run_id: "run_valid" } });
if (fetches !== 1) throw new Error("valid refresh request did not reach runtime fetch");
