...
func TestScannerBuiltin(t *testing.T) {
	s := NewScanner()
	for _, c := range BuiltinChecks() { s.AddCheck(c) }
	report := s.Scan()
	if report.Passed+report.Failed != len(BuiltinChecks()) { t.Error("check count") }
	if report.Score > 100 { t.Error("score") }
}
...
func TestVulnScanner(t *testing.T) {
	...
	if len(found) != 1 { t.Error("should find 1 vuln") }
	if found[0].Package != "libfoo" { t.Error("pkg") }
}
