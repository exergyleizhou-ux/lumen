...
func TestAddSymbol(t *testing.T) {
	...
	if len(m.Symbols()) != 1 { t.Error("symbol count") }
}
func TestCalleesCallers(t *testing.T) {
	m := NewMap()
	m.AddCall("main", "foo", "main.go"); m.AddCall("main", "bar", "main.go")
	callees := m.Callees("main")
	if len(callees) != 2 { t.Error("callees") }
}
func TestCircularDeps(t *testing.T) {
	m := NewMap()
	m.AddImport("a", "b"); m.AddImport("b", "a")
	cycles := m.CircularDeps()
	if len(cycles) == 0 { t.Error("should detect cycle") }
}
