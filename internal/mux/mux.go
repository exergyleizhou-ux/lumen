...
func (m *Mux) OpenStream() (*Stream, error) {
	...
	if len(m.streams) >= m.maxStreams {
		return nil, fmt.Errorf("max streams %d reached", m.maxStreams)
	}
	id := atomic.AddInt64(&m.nextID, 1)
	s := &Stream{ID: id, State: "open", CreatedAt: time.Now()}
	m.streams[id] = s
	return s, nil
}
func (m *Mux) CloseStream(id int64) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	s, ok := m.streams[id]
	if !ok {
...
func (m *Mux) Send(streamID int64, data []byte) ([]byte, error) {
	...
	m.mu.Unlock()
	if !ok || s.State != "open" {
		return nil, fmt.Errorf("stream %d not open", streamID)
	}
	s.BytesIn += int64(len(data))
	if m.handler != nil {
		return m.handler(s, data), nil
	}
	return data, nil
}
func (m *Mux) StreamCount() int { m.mu.Lock(); defer m.mu.Unlock(); return len(m.streams) }
func (m *Mux) FormatStreams() string {
	m.mu.Lock()
	defer m.mu.Unlock()
