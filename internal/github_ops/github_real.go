...
func NewClient(owner, repo string) *Client {
	...
	if token == "" {
		...
		if err == nil { token = strings.TrimSpace(string(out)) }
	}
	return &Client{
		token:   token,
		owner:   owner,
...
func (c *Client) request(ctx context.Context, method, path string, body []byte) ([]byte, error) {
	...
	if body != nil { reqBody = bytes.NewReader(body) }
...
func (c *Client) ListPRs(ctx context.Context, state string) ([]PRInfo, error) {
	...
	if state == "" { path = fmt.Sprintf("/repos/%s/%s/pulls?per_page=30", c.owner, c.repo) }
	resp, err := c.get(ctx, path)
	if err != nil { return nil, err }
	var prs []PRInfo
	if err := json.Unmarshal(resp, &prs); err != nil { return nil, err }
...
func parsePR(data []byte) (*PRInfo, error) {
	...
	if u, ok := raw["user"].(map[string]any); ok { pr.User, _ = u["login"].(string) }
	if h, ok := raw["head"].(map[string]any); ok { pr.Branch, _ = h["ref"].(string) }
	if b, ok := raw["base"].(map[string]any); ok { pr.Base, _ = b["ref"].(string) }
	if body, ok := raw["body"].(string); ok { pr.Body = body }
	return pr, nil
}
...
func (c *Client) CreateIssue(ctx context.Context, title, body string, labels []string) (*IssueInfo, error) {
	...
	var raw map[string]any
	json.Unmarshal(resp, &raw)
	if u, ok := raw["user"].(map[string]any); ok { issue.User, _ = u["login"].(string) }
	issue.Number = int(raw["number"].(float64))
	var lbls []string
	if rawLbls, ok := raw["labels"].([]any); ok {
		for _, l := range rawLbls {
			if lm, ok := l.(map[string]any); ok { lbls = append(lbls, lm["name"].(string)) }
		}
	}
	issue.Labels = lbls
	return &issue, nil
...
func (c *Client) ListIssues(ctx context.Context, state string, labels []string) ([]IssueInfo, error) {
	...
	if len(labels) > 0 { path += "&labels=" + strings.Join(labels, ",") }
	resp, err := c.get(ctx, path)
	if err != nil { return nil, err }
...
func (c *Client) GetCIStatus(ctx context.Context, ref string) (*CIStatus, error) {
	...
	if err := json.Unmarshal(resp, &raw); err != nil { return nil, err }
	...
	for _, cr := range raw.CheckRuns {
		...
		if cr.CompletedAt != nil { check.CompletedAt = *cr.CompletedAt }
		ci.Checks = append(ci.Checks, check)
		if cr.Conclusion == "success" { successCount++ }
	}
	if successCount == len(ci.Checks) && len(ci.Checks) > 0 { ci.State = "success" } else if successCount < len(ci.Checks) { ci.State = "failure" }
	return ci, nil
}
...
func (c *Client) WaitCI(ctx context.Context, ref string, timeout time.Duration) (*CIStatus, error) {
	...
	for time.Now().Before(deadline) {
		...
		if err != nil { return nil, err }
		if ci.State == "success" || ci.State == "failure" { return ci, nil }
		select {
		case <-ctx.Done(): return ci, ctx.Err()
...
func (c *Client) SearchCode(ctx context.Context, query string) ([]CodeSearchResult, error) {
	...
	for _, item := range raw.Items {
		...
		for _, tm := range item.TextMatches { fragments = append(fragments, tm.Fragment) }
...
func (c *Client) CreatePRViaCLI(title, body, head, base string) (*PRInfo, error) {
	...
	if err != nil { return nil, fmt.Errorf("gh pr create: %w: %s", err, output) }
...
func FormatPRs(prs []PRInfo) string {
	...
	for _, pr := range prs {
		icon := "🟢"; switch pr.State { case "closed": icon = "🔴"; case "merged": icon = "🟣" }
...
func FormatCI(ci *CIStatus) string {
	var sb strings.Builder
	icon := "🟡"; if ci.State == "success" { icon = "✅" } else if ci.State == "failure" { icon = "🔴" }
	fmt.Fprintf(&sb, "CI: %s %s (%d checks):\n%s\n\n", icon, ci.State, ci.TotalCount, strings.Repeat("─", 60))
	for _, c := range ci.Checks {
		checkIcon := "🟡"; if c.Conclusion == "success" { checkIcon = "✅" } else if c.Conclusion == "failure" { checkIcon = "🔴" }
...
func FormatIssues(issues []IssueInfo) string {
	...
	for _, is := range issues {
		icon := "🟢"; if is.State == "closed" { icon = "⚫" }
		tagStr := ""
		if len(is.Labels) > 0 { tagStr = " [" + strings.Join(is.Labels, ", ") + "]" }
