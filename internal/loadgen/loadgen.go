...
type Config struct {
	...
	TargetFn       func(int64) error
}

func DefaultConfig() Config {
	return Config{Name: "default", Concurrency: 10, TotalRequests: 1000, RampTime: 100 * time.Millisecond, RequestTimeout: 30 * time.Second}
}
...
type LatencyStats struct {
	Min, Max, Mean, Median, P95, P99 time.Duration
...
func computeLatStats(s []time.Duration) LatencyStats {
	...
	ls.Mean = time.Duration(sum / int64(len(s)))
	ls.Median = s[len(s)/2]
	ls.P95 = s[int(math.Ceil(0.95*float64(len(s))))-1]
	ls.P99 = s[int(math.Ceil(0.99*float64(len(s))))-1]
	return ls
}
...
func (r *Runner) Run(cfg Config) (*Report, error) {
	if cfg.TargetFn == nil {
		return nil, fmt.Errorf("TargetFn required")
	}
	...
	var latMu, errMu sync.Mutex
	var wg sync.WaitGroup
	sem := make(chan struct{}, cfg.Concurrency)
	start := time.Now()
	for i := int64(0); i < cfg.TotalRequests; i++ {
		sem <- struct{}{}
		wg.Add(1)
		go func(id int64) {
			defer wg.Done()
	...
	report.Total = cfg.TotalRequests
	report.Success = succ.Load()
	report.Failed = fail.Load()
	report.Throughput = float64(cfg.TotalRequests) / elapsed.Seconds()
	report.Latencies = computeLatStats(lats)
	r.mu.Lock()
