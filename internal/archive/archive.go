...
func (s *Snapshot) AddFile(path string, data []byte) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.entries = append(s.entries, Entry{Name: path, Size: int64(len(data)), Data: data, ModTime: time.Now()})
}
func (s *Snapshot) AddDir(path string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.entries = append(s.entries, Entry{Name: path, IsDir: true, ModTime: time.Now()})
}
...
func (s *Snapshot) Format() string {
	...
	defer s.mu.Unlock()
	var sb strings.Builder
	fmt.Fprintf(&sb, "Snapshot (%d entries):\n%s\n\n", len(s.entries), strings.Repeat("─", 40))
