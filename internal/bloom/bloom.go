...
func New(size uint64, hashCount uint) *Filter {
	...
	return &Filter{bits: make([]uint64, (size+63)/64), m: size, k: hashCount}
}
...
func (f *Filter) EstimatedItems() float64 {
	f.mu.RLock()
	defer f.mu.RUnlock()
	return f.estimatedItemsLocked()
}

// estimatedItemsLocked assumes the caller holds the lock.
func (f *Filter) estimatedItemsLocked() float64 {
	ones := 0
	for _, w := range f.bits {
		ones += popcount64(w)
	}
	p := float64(ones) / float64(f.m)
	if p >= 1 {
		return float64(f.m)
	}
	return -float64(f.m) / float64(f.k) * math.Log(1-p)
}
...
func (f *Filter) Format() string {
	...
	pct := float64(ones) / float64(f.m) * 100
	return fmt.Sprintf("Bloom Filter: %d bits, %d hashes, %.1f%% filled, ~%.0f items", f.m, f.k, pct, f.estimatedItemsLocked())
}
