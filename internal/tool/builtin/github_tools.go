...
func (t *GitHubPRCreateTool) Description() string { return "Create a GitHub Pull Request. Requires gh CLI auth or GITHUB_TOKEN." }
...
func (t *GitHubPRListTool) Description() string { return "List open Pull Requests for a GitHub repository." }
...
func (t *GitHubPRListTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if err != nil { return "", fmt.Errorf("list PRs: %w", err) }
...
func (t *GitHubCIStatusTool) Description() string { return "Check CI/CD status for a commit ref on GitHub." }
...
func (t *GitHubCIStatusTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if err != nil { return "", fmt.Errorf("CI status: %w", err) }
...
func (t *GitHubIssueCreateTool) Description() string { return "Create a GitHub Issue with optional labels." }
...
func (t *GitHubIssueCreateTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	var p struct{ Owner, Repo, Title, Body string; Labels []string }
	if err := json.Unmarshal(args, &p); err != nil { return "", err }
	client := github_ops.NewClient(p.Owner, p.Repo)
	is, err := client.CreateIssue(ctx, p.Title, p.Body, p.Labels)
	if err != nil { return "", fmt.Errorf("create issue: %w", err) }
...
func (t *GitHubSearchCodeTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if err != nil { return "", fmt.Errorf("search code: %w", err) }
