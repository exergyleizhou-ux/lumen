...
func (t *ScreenCaptureTool) Description() string { return "Take a screenshot of the entire screen. Returns the file path to the PNG. Use this to see what the user is seeing." }
func (t *ScreenCaptureTool) Schema() json.RawMessage { return json.RawMessage(`{"type":"object","properties":{"path":{"type":"string","description":"Optional file path. If empty, saves to a temp file."}}}`) }
...
func (t *MouseClickTool) Description() string { return "Click the mouse at given coordinates (or current position). Use screen_capture first to see the screen, then click on elements." }
...
func (t *MouseClickTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	var p struct{ X, Y int; Button string }
	if err := json.Unmarshal(args, &p); err != nil { return "", err }
	switch p.Button {
	case "right":
		if err := computeruse.RightClick(p.X, p.Y); err != nil { return "", err }
		return fmt.Sprintf("Right-clicked at (%d, %d)", p.X, p.Y), nil
	case "double":
		if err := computeruse.DoubleClick(p.X, p.Y); err != nil { return "", err }
		return fmt.Sprintf("Double-clicked at (%d, %d)", p.X, p.Y), nil
	default:
		if err := computeruse.Click(p.X, p.Y); err != nil { return "", err }
...
func (t *TypeTextTool) Description() string { return "Type text into the currently focused field. Use after clicking on a text field." }
func (t *TypeTextTool) Schema() json.RawMessage { return json.RawMessage(`{"type":"object","properties":{"text":{"type":"string"}},"required":["text"]}`) }
func (t *TypeTextTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	var p struct{ Text string }
	if err := json.Unmarshal(args, &p); err != nil { return "", err }
	if err := computeruse.TypeText(p.Text); err != nil { return "", err }
...
func (t *KeyPressTool) Description() string { return "Press a key or key combination. Examples: 'return', 'escape', 'command+q', 'command+v', 'shift+tab'." }
func (t *KeyPressTool) Schema() json.RawMessage { return json.RawMessage(`{"type":"object","properties":{"combo":{"type":"string","description":"Key or combo like return, escape, command+v"}},"required":["combo"]}`) }
func (t *KeyPressTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	var p struct{ Combo string }
	if err := json.Unmarshal(args, &p); err != nil { return "", err }
	if err := computeruse.KeyPress(p.Combo); err != nil { return "", err }
...
func (t *OpenAppTool) Description() string { return "Launch or activate a macOS application by name. E.g. 'Safari', 'Terminal', 'Finder', 'Visual Studio Code'." }
func (t *OpenAppTool) Schema() json.RawMessage { return json.RawMessage(`{"type":"object","properties":{"app":{"type":"string"}},"required":["app"]}`) }
func (t *OpenAppTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	var p struct{ App string }
	if err := json.Unmarshal(args, &p); err != nil { return "", err }
	if err := computeruse.OpenApp(p.App); err != nil { return "", err }
...
func (t *UIInspectTool) Description() string { return "Inspect the current window for UI elements via macOS Accessibility API. Returns roles, titles, positions. Optionally filter by role (button, text field, window, etc). Requires Accessibility permission." }
func (t *UIInspectTool) Schema() json.RawMessage { return json.RawMessage(`{"type":"object","properties":{"role":{"type":"string","description":"Filter by UI role: button, text field, window, menu, etc. Empty for all elements."}}}`) }
func (t *UIInspectTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if err != nil { return "", err }
	if len(elements) == 0 { return "No matching UI elements found. Make sure Accessibility permission is granted in System Settings.", nil }
	...
	for _, e := range elements {
		...
		if e.Focused { sb.WriteString(" [FOCUSED]") }
		sb.WriteByte('\n')
	}
...
func (t *ComputerStatusTool) Description() string { return "Get current computer status: screen size, mouse position, active application, running apps." }
func (t *ComputerStatusTool) Schema() json.RawMessage { return json.RawMessage(`{"type":"object","properties":{}}`) }
