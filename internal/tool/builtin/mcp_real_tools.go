...
func (t *MCPConnectTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	key := p.Command
	if len(p.Args) > 0 {
		key = p.Command + " " + strings.Join(p.Args, " ")
	}
...
func (t *MCPCallToolTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if sb.Len() == 0 {
		return wrapMCPAgentOutput(p.Server+":"+p.Tool, "(no output)"), nil
	}
...
func (t *MCPListResourcesTool) Description() string {
	return "List resources exposed by a connected MCP server."
}
...
func (t *MCPListPromptsTool) Description() string {
	return "List prompt templates exposed by a connected MCP server."
}
...
func getMCPClient(key string) *mcplife.Client {
	if key != "" {
		return mcpClientByKey(key)
	}
	for _, c := range mcpClientsSnapshot() {
		return c
	}
	return nil
}
