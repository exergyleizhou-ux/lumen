...
func (t *CronParseTool) Description() string { return "Parse a 5-field cron expression and show next 5 execution times." }
...
func (t *CronNextNTool) Description() string { return "Get the next N execution times for a cron expression from now." }
...
func (t *CronNextNTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	var p struct{ Expression string; Count int }
	if err := json.Unmarshal(args, &p); err != nil { return "", err }
...
func (t *CronDescribeTool) Description() string { return "Convert a cron expression to human-readable English." }
