...
func (t *SchemaRegisterTool) Description() string {
	return "Register a new schema version for a subject with format validation."
}
...
func (t *SchemaCheckCompatTool) Description() string {
	return "Check backward compatibility between two schema versions."
}
...
func (t *SchemaCheckCompatTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if len(issues) == 0 {
		return "✅ Compatible: no breaking changes detected.", nil
	}
...
func (t *SchemaListTool) Description() string {
	return "List all registered schema subjects and versions."
}
func (t *SchemaListTool) Schema() json.RawMessage {
