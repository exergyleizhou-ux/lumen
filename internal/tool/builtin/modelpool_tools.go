...
func (t *LLMChatTool) Description() string {
	return "Send a chat request to a registered LLM model (OpenAI or Anthropic). Returns the model's response and token usage."
}
...
func (t *LLMChatTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	for _, cfg := range modelpool.DefaultModelConfigs() {
		client.RegisterModel(cfg)
	}
...
func (t *LLMCostTool) Description() string {
	return "Show accumulated LLM cost and usage report across all models."
}
func (t *LLMCostTool) Schema() json.RawMessage {
...
func (t *LLMStreamTool) Description() string {
	return "Stream a chat response from a registered LLM model token by token."
}
