...
func (t *ConfigGetTool) Schema() json.RawMessage {
...
func (t *ConfigGetTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	v, ok := s.Get(p.Key)
	if !ok {
		return fmt.Sprintf("Key %q not found. Available keys: %v", p.Key, s.Keys()), nil
	}
...
func (t *ConfigSetTool) Description() string {
	return "Set a live configuration value with hot-reload notification."
}
...
func (t *ConfigHistoryTool) Schema() json.RawMessage {
	return json.RawMessage(`{"type":"object","properties":{"limit":{"type":"integer","default":20}}}`)
}
...
func (t *EnvListTool) Description() string {
	return "List all environment variables with secrets masked."
}
func (t *EnvListTool) Schema() json.RawMessage {
...
func (t *BlueprintBuildTool) Description() string {
	return "Build and wire components from a blueprint definition."
}
...
func (t *BlueprintBuildTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if p.Blueprint != "default" {
		return "", fmt.Errorf("blueprint %q not found (available: default)", p.Blueprint)
	}
...
func (t *BlueprintValidateTool) Description() string {
	return "Validate a blueprint for dependency and structural errors."
}
...
func (t *BlueprintValidateTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	errs := r.Validate(bp)
	if len(errs) == 0 {
		return "✅ Blueprint valid: no issues found.", nil
	}
	var msgs []string
	for _, e := range errs {
