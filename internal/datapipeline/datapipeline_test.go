...
func TestPipelineFilter(t *testing.T) {
	...
	var records []Record
	for i := 0; i < 10; i++ {
		records = append(records, Record{"id": i, "val": i})
	}
...
func TestCSVSourceSink(t *testing.T) {
	src := NewCSVSource("test.csv")
	_, err := src.Read()
	if err == nil {
		t.Log("no file — expected error")
	}
}

func TestFormatRecords(t *testing.T) {
	s := FormatRecords([]Record{{"a": 1}})
	if !strings.Contains(s, "1 record") {
		t.Error("format")
	}
}
