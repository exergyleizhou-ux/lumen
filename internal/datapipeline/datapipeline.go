...
func NewFilterStage(name string, fn func(Record) bool) *FilterStage {
	return &FilterStage{name: name, fn: fn}
}
...
type AggregateStage struct {
	name    string
	groupBy string
	aggFn   func([]Record) Record
}

func NewAggregateStage(name, groupBy string, fn func([]Record) Record) *AggregateStage {
	return &AggregateStage{name: name, groupBy: groupBy, aggFn: fn}
}
func (a *AggregateStage) Name() string { return a.name }
func (a *AggregateStage) Process(in []Record) ([]Record, error) {
	groups := map[string][]Record{}
	for _, r := range in {
		key := fmt.Sprint(r[a.groupBy])
		groups[key] = append(groups[key], r)
	}
	var out []Record
	for _, grp := range groups {
		out = append(out, a.aggFn(grp))
	}
...
func NewSortStage(name, key string, desc bool) *SortStage {
	return &SortStage{name: name, key: key, desc: desc}
}
