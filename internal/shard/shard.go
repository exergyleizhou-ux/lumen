...
func NewRing(virtualNodes int) *Ring {
	...
	return &Ring{nodes: map[string]*Node{}, virtualNodes: virtualNodes}
}
...
func (r *Ring) MarkHealthy(id string, healthy bool) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Ring) GetReplicas(key string, count int) []*Node {
	...
	for i := 0; i < len(r.ring) && len(result) < count; i++ {
		entryIdx := (idx + i) % len(r.ring)
		nodeID := r.ring[entryIdx].nodeID
		if !seen[nodeID] {
			seen[nodeID] = true
...
func (r *Ring) rebuild() {
	...
	for id, n := range r.nodes {
		...
		for v := 0; v < r.virtualNodes*n.Weight; v++ {
			h := hashKey(fmt.Sprintf("%s-v%d", id, v))
			r.ring = append(r.ring, ringEntry{hash: h, nodeID: id})
		}
	}
...
func (kd *KeyDistribution) Record(nodeID string) {
	kd.mu.Lock()
	defer kd.mu.Unlock()
	kd.counts[nodeID]++
}
func (kd *KeyDistribution) Stats() map[string]int {
	kd.mu.Lock()
	defer kd.mu.Unlock()
	out := make(map[string]int)
	for k, v := range kd.counts {
		out[k] = v
	}
	return out
}
func (kd *KeyDistribution) Imbalance() float64 {
	kd.mu.Lock()
	defer kd.mu.Unlock()
	if len(kd.counts) == 0 {
		return 0
	...
	avg := float64(sum) / float64(len(kd.counts))
	return (float64(max) - float64(min)) / avg
}

func (r *Ring) FormatRing() string {
	r.mu.RLock()
	defer r.mu.RUnlock()
	var sb strings.Builder
	fmt.Fprintf(&sb, "Consistent Hash Ring: %d nodes, %d vnodes\n%s\n\n", len(r.nodes), len(r.ring), strings.Repeat("─", 50))
	...
	for _, id := range ids {
		...
		fmt.Fprintf(&sb, "  %s %-20s %-30s weight=%d vnodes=%d\n", icon, id, n.Address, n.Weight, vnodes)
	}
	return sb.String()
}
