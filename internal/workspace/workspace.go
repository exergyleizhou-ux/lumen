...
func (d *Detector) Detect() (*Info, error) {
	...
	for f, t := range tools {
		if _, err := os.Stat(filepath.Join(root, f)); err == nil {
			info.BuildTools = append(info.BuildTools, t)
		}
	}
...
func (i *Info) Format() string {
	...
	if len(i.BuildTools) > 0 {
		fmt.Fprintf(&sb, "Build tools: %s\n", strings.Join(i.BuildTools, ", "))
	}
...
func GitStatus(root string) (*GitInfo, error) {
	info := &GitInfo{}
	if data, err := runGit(root, "rev-parse", "--abbrev-ref", "HEAD"); err == nil {
		info.Branch = strings.TrimSpace(data)
	}
	if data, err := runGit(root, "remote", "get-url", "origin"); err == nil {
		info.Remote = strings.TrimSpace(data)
	}
	if data, err := runGit(root, "diff", "--name-only"); err == nil {
		info.ModifiedFiles = countLines(data)
	}
	if data, err := runGit(root, "diff", "--cached", "--name-only"); err == nil {
		info.StagedFiles = countLines(data)
	}
	if data, err := runGit(root, "ls-files", "--others", "--exclude-standard"); err == nil {
		info.UntrackedFiles = countLines(data)
	}
	return info, nil
}
