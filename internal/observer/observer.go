...
func (lc *LogCorrelator) Log(traceID, spanID, level, message string) {
	...
	if len(lc.logs[traceID]) > lc.maxLogs {
		lc.logs[traceID] = lc.logs[traceID][1:]
	}
}
...
func (sc *SampleCollector) Record(name string, value float64) {
	...
	if len(sc.samples[name]) > 1000 {
		sc.samples[name] = sc.samples[name][len(sc.samples[name])-1000:]
	}
}
