...
func TestRunnerMigrate(t *testing.T) {
	...
	for _, s := range statuses {
		if !s.Applied {
			t.Errorf("v%d should be applied", s.Version)
		}
	}
	if r.Pending() != 0 {
		t.Error("no pending")
...
func TestRunnerRollback(t *testing.T) {
	...
	if len(statuses) != 1 {
		t.Error("rollback count")
	}
	if r.Pending() != 1 {
		t.Error("should be pending after rollback")
	}
}
func TestDuplicateVersion(t *testing.T) {
	r := NewRunner()
	if err := r.Register(&Migration{Version: 1, Name: "m1"}); err != nil {
		t.Fatal(err)
	}
	if err := r.Register(&Migration{Version: 1, Name: "dup"}); err == nil {
