...
async fn run_headless_with_home_env(
...
async fn test_headless_tools_allowlist_does_not_fail_open_for_disabled_web_fetch() {
    ...
    let result = run_headless_with_home_env(
        &server,
        &[
            "-p",
            "say hello",
...
async fn test_stdio_science_seq_analyze_restart_reprompts_exact_pending_once() {
    with_local_set(|| async {
        ...
        let mut first = GrokStdioClient::spawn_with_home(
            &server,
            &workspace,
            tempfile::TempDir::new().expect("restart test home"),
        )
        .await;
...
async fn test_stdio_prompt_then_immediate_load_session() {
    with_local_set(|| async {
        let server = MockInferenceServer::start().await.expect("start mock server");
        let workdir = git_workdir();
        let mut writer = GrokStdioClient::spawn_with_home(
            &server,
            workdir.path(),
            tempfile::TempDir::new().expect("prompt-then-load test home"),
        )
        .await;
...
struct ConfigTestHarness {
    ...
    workdir: xai_grok_test_support::TestSandbox,
