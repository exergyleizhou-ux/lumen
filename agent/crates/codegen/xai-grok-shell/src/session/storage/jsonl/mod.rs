...
impl JsonlStorageAdapter {
    ...
    /// crash-atomic (`append_jsonl_line`), so a torn line must not fail the
    /// whole load — the same corruption class already bricked resume for
    /// `updates.jsonl` / `chat_history.jsonl` before their readers were made
    /// lenient. Rewrite paths (merge/truncate) must keep using the strict
    /// reader: they persist what they read, and silently dropping an
    /// unparseable line there would destroy on-disk bytes.
    ...
    /// STRICT load for the rewrite paths (merge/truncate): they persist what
    /// they read, so an unparseable line must abort before the write instead
    /// of being silently dropped from disk.
