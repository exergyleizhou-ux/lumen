...
/// Run the single, durable session shutdown path used by an explicit shutdown
/// and by an actor-approved idle unload. Keeping this in the actor avoids a
/// separate `IsBusy` → `Shutdown` round trip where a newly queued prompt could
/// be lost between the two commands.
async fn shutdown_session(session: &SessionActor, replay_buffer: &mut ReplayBuffer) {
    shutdown_workflows(session).await;
    // Flush the actor-owned replay buffer so any streamed chunks still pending
    // at shutdown (e.g. reasoning text from a sampler stream racing with a CLI
    // exit / harness teardown) are committed to updates.jsonl before the
    // session directory is snapshotted for trace upload. Mirrors the same
    // flush in the Cancel, CopyFile, and FlushComplete arms.
    if let Some(notification) = replay_buffer.flush() {
        session.emit_buffered(notification).await;
    }
    // Drop any queued synthetic auto-wake prompts and pending notifications
    // before running hooks. Without this, a synthetic prompt that slipped
    // through the per-tool-result sweep could still get flushed to
    // chat_history.jsonl by a later persistence path, producing a trailing
    // `<system-reminder>` with no assistant reply. Placed BEFORE hook dispatch
    // so the cleanup runs even if hooks abort.
    ...
    // Memory: save session summary before shutdown. Subagent sessions are
    // deliberately excluded: their task-tree ledger is governed separately
    // and they cannot directly promote long-term memory.
        ...
        if let Some(storage) = session.memory.storage() {
            ...
            if let crate::session::memory::hooks::SessionEndResult::Written(ref path_str) = result {
                session
                    ...
                    .await;
                session
                    .send_xai_notification(XaiSessionUpdate::MemorySessionSaved {
                        path: path_str.clone(),
                    })
                    .await;
            }
        ...
        tokio::select! {
                ...
                event = chat_state_event_rx.recv() => {
                    match event {
                        ...
                        Some(xai_chat_state::ChatStateEvent::HistoryMutationCommitted {
                            revision,
                            mutation,
                            new_len,
                        }) => {
                            match session
                                .rotate_cache_epoch_after_history_mutation(mutation)
                                .await
                            {
                                Ok(record) => tracing::debug!(
                                    ...
                                    "chat history mutation committed and cache epoch rotated"
                                ),
                ...
                maybe_cmd = cmd_rx.recv() => {
                    ...
                    match cmd {
                        ...
                        SessionCommand::BeginScienceCsv(command) => {
                            let command = *command;
                            let result = session.prepare_science_csv(
                                command.store,
                                command.context,
                        ...
                        SessionCommand::FinishScienceCsv(command) => {
                            let command = *command;
                            let result = Box::pin(session.finish_science_csv(
                                command.prepared,
                                command.decision,
                                command.reason,
                            ))
                        ...
                        SessionCommand::BeginScienceSshScpAdmission(command) => {
                            let command = *command;
                            let result = session.prepare_science_ssh_scp_admission(
                                command.store,
                                command.context,
                                command.policy,
                                command.request,
                        ...
                        SessionCommand::FinishScienceSshScpAdmission(command) => {
                            let command = *command;
                            let result = session.finish_science_ssh_scp_admission(
                                command.prepared,
                                command.decision,
                            );
                            let _ = command.respond_to.send(result);
                        }
                        SessionCommand::ExecuteScienceSshScpOfflineTransport(command) => {
                            let command = *command;
                            let result = session.execute_science_ssh_scp_offline_transport(
                                command.store,
                                command.ticket,
                                command.outcome,
                            );
                        ...
                        SessionCommand::ExecuteScienceSshScpTransport(command) => {
                            let command = *command;
                            let result = session.execute_science_ssh_scp_transport(
                                command.store,
                                command.ticket,
                                command.operation,
                                command.config,
                        ...
                        SessionCommand::VerifyScienceGoal(command) => {
                            let command = *command;
                            let result = session
                                .verify_science_goal(command.store, command.run_id)
                                .await;
                            let _ = command.respond_to.send(result);
                        }
                        SessionCommand::BeginScienceImport(command) => {
                            let command = *command;
                            let result = session.prepare_science_import(
                                command.store,
                                command.context,
                                command.source_path,
                                command.bytes,
                        ...
                        SessionCommand::FinishScienceImport(command) => {
                            let command = *command;
                            let result = Box::pin(session.finish_science_import(
                                command.prepared,
                                command.decision,
                                command.reason,
                            ))
                        ...
                        SessionCommand::FinishScienceFetch(command) => {
                            let command = *command;
                            let result = Box::pin(session.finish_science_fetch(
                                command.prepared,
                                command.decision,
                                command.reason,
                            ))
                        ...
                        SessionCommand::Prompt { prompt_id, prompt_blocks, prompt_mode, artifact_upload_ctx, client_identifier, screen_mode, verbatim, traceparent, json_schema, send_now, admission, tool_overrides_update, respond_to, persist_ack, parsed_prompt_tx } => {
                            ...
                            let cancel_for_send_now = session
                                .queue_input(prompt_blocks, prompt_id, prompt_mode, trace_gcs_config, artifact_tracker, client_identifier, screen_mode, verbatim, json_schema, send_now || expert_off, task_wake_fallback, tool_overrides_update, respond_to, persist_ack, parsed_prompt_tx)
                                .await;
                            if cancel_for_send_now {
                                if expert_off {
                                    session.request_expert_disable().await;
                                    if let Some(notification) = replay_buffer.flush() {
                                        session.emit_buffered(notification).await;
                                    }
                                    session.pending_interjections.clear();
                                    session
                                        .cancel_running_task(false, false, false, Some("expert_off".to_string()))
                                        .await;
                                    let _ = session.restore_disabled_expert().await;
                                    session.state.lock().await.notifications_suppressed = false;
                                } else {
                        ...
                        SessionCommand::SetSessionModel { sampling_config, use_concise, apply_prompt_override, skip_prompt_rewrite, auto_compact_threshold_percent, responds_to } => {
                            let updated_model_id = session.handle_external_set_session_model(sampling_config, use_concise, apply_prompt_override, skip_prompt_rewrite, auto_compact_threshold_percent).await;
                        ...
                        SessionCommand::UnloadIfIdle { respond_to } => {
                            // This is deliberately one actor-mailbox operation:
                            // a Prompt processed before it makes `state` busy;
                            ...
                            shutdown_session(&session, &mut replay_buffer).await;
                            return;
                        }
